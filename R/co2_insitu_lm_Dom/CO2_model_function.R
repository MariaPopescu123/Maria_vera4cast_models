...
Dom_CO2model_function <- function(forecast_date, model_id) {
  ...
  for(i in 1:length(focal_sites)) {  
    ...
    fit <- lm(co2 ~ co2_yday + air_temperature + Chla_ugL_mean + CO2flux_umolm2s_mean, data = site_target)
    ...
    # 3.7 is your empirical error calculation!
    ...
    param_df <- data.frame(
      beta1 = rnorm(n_members, coeffs[1], params_se[1]), beta2 = rnorm(n_members, coeffs[2], params_se[2]), 
      beta3 = rnorm(n_members, coeffs[3], params_se[3]), beta4 = rnorm(n_members, coeffs[4], params_se[4]),
    ...
    for(d in 2:length(forecasted_dates)) {
      ...
      temp_pred$value <- pmax(param_df$beta1 + temp_lag$value * param_df$beta2 + temp_driv_31$air_temperature * param_df$beta3 + temp_driv_31$Chla_ugL_mean * param_df$beta4 + temp_driv_31$CO2flux_umolm2s_mean * param_df$beta5, 0)
